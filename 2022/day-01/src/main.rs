...
fn main() -> Result<(), std::io::Error> {
    ...
    let v: Vec<Vec<i32>> = content.split("\n\n").map(|elf_bag| {
        elf_bag.split('\n').map(|num| num.parse::<i32>().unwrap_or(0)).collect()
    }).collect();
                                           // collect::<Vec<&str>>();
    ...
    let result = v.iter().map(|bag| bag.iter().sum::<i32>()).collect::<Vec<i32>>()
                          .iter().fold(vec![0, 0, 0], |mut acc, &num| {
                if num > acc[0] {
                    acc[2] = acc[1];
                    acc[1] = acc[0];
                    acc[0] = num;
